...
# Optional generated conf fragment (conf/<key>-autogen.conf), empty if
# absent. Sets CONFIG_BOOT_MAX_IMG_SECTORS for every board (swap boards get
# max(slot0, slot1); single-app boards pin slot0's count; slot1-on-external-
# SPI-NOR boards also pin the NOR driver's layout page size). Generated by
...
ifneq ($(strip $(BOARD_AUTOGEN_CONF)),)
...
help:
	...
	@echo "                           (a .uf2 of it on UF2-capable boards, and a slot1 test-"
	@echo "                           upgrade variant on swap-capable boards)"
	@echo "  make uf2 BOARD=<key>     alias for 'make updater' (kept for convenience)"
...
# On UF2-capable boards (conf/<key>.conf enables CONFIG_MCUBOOT_UF2=y) the
# signed image is also converted to a .uf2 (see the `uf2` target below):
...
#
# The UF2 base address is slot0's flash offset (fa_off) -- the address the UF2
# bootloader writes incoming blocks at -- and the family ID is the
# bootloader's CONFIG_MCUBOOT_UF2_FAMILY_ID; both are read from the freshly
# built trees by tools/uf2_updater.py (base from the updater EDT pickle,
# family from the bootloader .config). They are computed inside the recipe
# (shell `$$()`, not Make's `$(shell)`) so they read the build dirs *after*
# `updater` has populated them. Conversion uses the vendored upstream
# uf2conv.py (tools/uf2conv.py, from microsoft/uf2); `-c` makes it write a
# file instead of flashing a drive.
...
# Build the updater (including its .uf2 on UF2-capable boards). Kept as a
# convenience alias so `make uf2 BOARD=<key>` and `make all-uf2` keep working;
# all the work -- bootloader, updater, signing and UF2 conversion -- happens
# in the `updater` target above.
uf2:
