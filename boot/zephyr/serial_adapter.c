...
boot_uart_fifo_init(void)
{
	...
	/* Brings up the shared bootloader USB device (idempotent). When
	 * UF2 is enabled too, the USB device is already up with both the
	 * mass storage and the CDC ACM classes on it. */
	rc = boot_usb_enable();
	if (rc) {
		return (-1);
	}

#ifndef CONFIG_MCUBOOT_UF2
	/* Feed WDT while waiting for USB host to open the serial port.
	 * Without this, the hardware watchdog fires before the serial
	 * recovery loop (which feeds the WDT) is reached. In the combined
	 * UF2 + serial update mode this wait is skipped: USB is already up
	 * and the update mode entry must not block on the host opening the
	 * serial port (the UF2 drive has to be usable meanwhile). */
